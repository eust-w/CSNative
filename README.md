...
最新 macOS 构建在 [GitHub Releases](https://github.com/eust-w/CSNative/releases/latest) 下载。每个 release 都包含：

- `CSNative-v*-darwin-*.zip`：压缩后的 macOS `.app`。
- `checksums.txt`：SHA-256 校验文件。

推送 `v*` tag 会触发自动发布流程：测试、`go vet`、Wails 构建、Release 上传，并同步发布 [`ghcr.io/eust-w/csnative-app`](https://github.com/eust-w/CSNative/pkgs/container/csnative-app) package。
